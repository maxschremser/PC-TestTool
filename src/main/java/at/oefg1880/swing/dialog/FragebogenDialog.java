 ...
 */
public class FragebogenDialog extends JDialog implements ActionListener, IConfig {
    TestToolFrame frame;
...
//        else if (e.getKeyChar() == KeyEvent.VK_SPACE) saveOrUpdate();
    ...
    public AntwortList getAntwortList() {
        if (list == null) {
            list = new AntwortList(frame, fragebogen);
    ...
    private void save() {
        FragebogenList list = frame.getFragebogenPane().getFragebogenList();
        list.add(
        ...
        if (frame.getFragebogenPane().getFragebogenList().getModel().getSize() > 0) {
            frame.getFragebogenPane().getSpeichernButton().setEnabled(true);
        }
        close();
    }
